...
levels(EvaluationDataByRegion$variable) <- c("Weighted", "Unweighted","EBP Unweighted","EBP Weighted", "MSE Estimate", "Sample Size")
EvaluationDataByRegion[,variable := factor(variable, levels = c("Unweighted","EBP Unweighted","Weighted", "EBP Weighted", "MSE Estimate", "Sample Size"))]
...
if(SaveBoxPlots){
      MSEVars <- c("Weighted", "Unweighted","EBP Unweighted","EBP Weighted")
      ...
      ggsave(filename = paste(PlotLocation, "BoxPlotMSEByDomain.png"), device = "png", plot = BoxPlotMSEByDomain, width = 18, height = 12, units = "cm")
      ...
      ggsave(filename = paste(PlotLocation, "BoxPlotMAEByDomain.png"), device = "png", plot = BoxPlotMAEByDomain, width = 18, height = 12, units = "cm")
      
      
      
      MSEEstimatesByDomain <- EvaluationDataByRegion[variable == "MSE Estimate", .(MSE_Estimate = value),by = .(Domain, Simulation)]
      MSEEstimatesByDomain <- merge(MSEEstimatesByDomain, MSEByDomain[variable == "EBP Weighted"], by = "Domain")
      ...
      ggsave(filename = paste(PlotLocation, "HistogramMSEEstimate.png"), device = "png", plot = HistogramMSEEstimate, width = 18, height = 12, units = "cm")
